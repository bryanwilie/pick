# taken from https://github.com/HLTCHKUST/KnowExpert/blob/main/evaluation.py
import os 
import re
import sys 
import argparse
...
"""
def _prec_recall_f1_score(pred_items, gold_items, measure):
    """
    PARLAI
...
"""
def _bigram_prec_recall_f1_score(pred_items, gold_items, measure):
    """
    PARLAI
    Computes precision, recall and f1 given a set of gold and prediction items.
    :param pred_items: iterable of predicted values
...
def get_unigram_F1(pred,gold):
    ...
    for p,g in zip(pred,gold):
        ...
        f1.append(_prec_recall_f1_score(p.split(" "),g.split(" "),'F1'))
    return np.mean(f1)

def get_bigram_F1(pred, gold):
    p = normalize_answer(pred)
    g = normalize_answer(gold)
    f1 = _bigram_prec_recall_f1_score(p, g,'F1')
    return f1

def get_unigram_precision(pred,gold):
    f1 = []
    for p,g in zip(pred,gold):
        p = normalize_answer(p)
        g = normalize_answer(g)
        f1.append(_prec_recall_f1_score(p.split(" "),g.split(" "),'precision'))
    return np.mean(f1)

def get_bigram_precision(pred, gold):
    p = normalize_answer(pred)
    g = normalize_answer(gold)
    f1 = _bigram_prec_recall_f1_score(p, g,'precision')
    return f1

def get_unigram_recall(pred,gold):
    f1 = []
    for p,g in zip(pred,gold):
        p = normalize_answer(p)
        g = normalize_answer(g)
        f1.append(_prec_recall_f1_score(p.split(" "),g.split(" "),'recall'))
    return np.mean(f1)

def get_bigram_recall(pred, gold):
    p = normalize_answer(pred)
    g = normalize_answer(gold)
    f1 = _bigram_prec_recall_f1_score(p, g,'recall')
    return f1
